...
public class HttpApi {
    ...
    private void confirm(HttpExchange x) throws IOException {
        ...
        if (in.has("tg_id") && plugin.storage().isBlocked(in.get("tg_id").getAsLong())) {
            send(x, 200, err("blocked"));
            return;
        }
    ...
    private void unlink(HttpExchange x) throws IOException {
        ...
        } else if (auth.isBanned(acc)) {
            plugin.storage().block(tgId, "бан аккаунта " + acc.name);
            o.addProperty("ok", false);
            o.addProperty("error", "blocked");
        } else {
